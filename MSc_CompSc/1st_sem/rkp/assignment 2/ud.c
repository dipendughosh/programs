	...
	while(ptr != NULL)
	{
		...
		if(f1 == 1)
		{
			do
			{
				if(c == 1)
				{
					printf("\nEnter the vertex adjacent to %d :- ",ptr->ver);
					scanf("%d",&a);
					c=0;
				}
				else
...
void edge_diff()
{
	...
	while(ptr != NULL)
	{
		...
		while(ptrr !=NULL)
		{
			if((ptr->dfsno)-(ptrr->vert->dfsno) == 1 || (ptr->dfsno)-(ptrr->vert->dfsno) == -1 || (ptr->dfscmpno)-(ptrr->vert->dfscmpno) == 1 || (ptr->dfscmpno)-(ptrr->vert->dfscmpno) == -1)
