...
public class SpaceInvader extends JFrame implements KeyListener, GLEventListener
{
	...
	public void stopGame()
	{
		...
		if(stopGame == 0)
		{
			...
			startBtn.setEnabled(true);*/
			stopGame = 1;
			result = JOptionPane.showOptionDialog(null, "Game Over!!!Score ::-"+hCntTxt.getText(),"Message", JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE, null, new String[] { "OK","EXIT" }, 5);
			if(result == 1)
			{
				System.exit(0);
			}
	...
	public void incrementHitCount()
	{
		...
		if(reset > 15)
		{
			try
			{
				...
				if(Integer.parseInt(cCntTxt.getText()) < 22 && result == 0)
				{	cCntTxt.setText(String.valueOf(Integer.parseInt(cCntTxt.getText())+3));
					reset=0;
				}
				shield=1;
				if(Integer.parseInt(cCntTxt.getText()) < 23 && result == 1)
	...
	public void decrementLife(int f)
	{
		if(result == 0)
		{
			if(Integer.parseInt(cCntTxt.getText()) > 0 && f==1)
			{
				...
				if(Integer.parseInt(cCntTxt.getText()) < 0)
					cCntTxt.setText("0");
			}
	...
	public int getLife()
	{
		...
		try
		{
			x = Integer.parseInt(cCntTxt.getText());
		}
		catch (Exception e)
		{
		}
