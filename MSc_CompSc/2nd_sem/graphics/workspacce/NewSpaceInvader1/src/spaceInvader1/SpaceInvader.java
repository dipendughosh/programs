		...
		synchronized (this) 
		{
			for (SpaceObjects object : objectSet) 
			{
				if(object instanceof Missiles)
				{
					...
					{
						if((object1 instanceof UFOS) &&  ((UFOS)object1).isHitted((Missiles)object))
						{
							((UFOS)object1).stopThread();
						}
					}
				}
			...
			if(flag == 0)
			{
				for (SpaceObjects object : objectSet) 
				{
					if(object instanceof Shooter)
					{
						for (SpaceObjects object1 : objectSet) 
						{
							if((object1 instanceof UFOS) &&  ((Shooter)object).isShooterHit((UFOS)object1))
							{
								((UFOS)object1).stopThread();
								decrementLife(1);
								flag=1;
...
/*	public void addListener(BGObject o)
	...
	public void incrementHitCount()
	{
		...
		if(reset > 15)
		{
			if(Integer.parseInt(cCntTxt.getText()) < 47)
