...
/**
 * This class is effectively a modified version of the Java ArrayList class.
 * It has been modified to permit public access to an internal array of
 * game objects. The class also supports sorting of game objects based upon
 * either their layer x or y offset (to permit overlap between objects to
...
/**
     ...
     * upon either their layer x or y location (the choice of x or y sorting should
     * depend upon if the game layer is wider than it is higher, etc.).
     */
    ...
    /**
     * Constructs an empty game object collection with an initial capacity
     * of ten.
     */
    ...
    /**
     * Constructs an empty game object collection with an initial
     * specified capacity.
     *
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Methods: Existing ArrayList methods                                   //
    ///////////////////////////////////////////////////////////////////////////
    
    /**
     * Trims the capacity of this instance to be the list's current size. An
     * application can use this operation to minimize the storage of the instance.
     */
     ...
     * at least one game object <tt>e</tt> such that
    ...
    public GameObject remove(int index) {
        ...
        gameObjects[--size] = null; // Let gc do its work
        return oldValue;
    }
    ...
    public boolean addAll(int index, Collection<GameObject> c) {
        ...
        if (numMoved > 0) {
            System.arraycopy(
                    gameObjects, index, gameObjects, index + numNew, numMoved);
        }
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Methods: Additional sorting methods                                   //
    ///////////////////////////////////////////////////////////////////////////

    /**
     * Set the sort order this game object collection to that specified.
     * 
     * @param sortOrder SortOrder to use for this collection
     */
    ...
    /**
     * Sort the game object contained within this game object collection 
     * based on the specified sort order.
     * <P>
     * Note: The built in Java sort algorithm is used to perform this
     * sort (i.e. merge/quick sort based). As such this method should be
     * called sort large numbers of out of sequence game object. It should
     * not be used to sort the game object collection following the addition
     * of a small number of game objects.
     */
    public void sortGameObjects() {
        // Ensure any space after size is correctly set to null
        for (int idx = size; idx < gameObjects.length; idx++) {
            gameObjects[idx] = null;
        }

        // Define a new comparator to be used to sort the game objects
        // based upon the specified sort order.
            ...
            comparator = new Comparator() {
                public int compare(Object comparator1, Object comparator2) {
                    if (comparator1 == null || comparator2 == null) {
                        ...
                        if (((GameObject) comparator1).x < ((GameObject) comparator2).x) {
                            return -1;
                        } else if (((GameObject) comparator1).x
                                > ((GameObject) comparator2).x) {
                            return 1;
                        } else {
                            return 0;
                        }
            ...
            comparator = new Comparator() {
                public int compare(Object comparator1, Object comparator2) {
                    if (comparator1 == null || comparator2 == null) {
                        ...
                        if (((GameObject) comparator1).y < ((GameObject) comparator2).y) {
                            return -1;
                        } else if (((GameObject) comparator1).y 
                                > ((GameObject) comparator2).y) {
                            return 1;
                        } else {
                            return 0;
                        }
     ...
     * few elements need to be 'corrected' - which will generally be the 
     * case for an existing population of games objects to which a few
     * objects are added.
    */    
    ...
    private void incrementalSortGameObjectsOnXPosition() {
        ...
        for (int i = 0; i < size - 1; i++) {
            if (gameObjects[i].x - gameObjects[i].boundingDimension / 2 
                    > gameObjects[i + 1].x - gameObjects[i + 1].boundingDimension / 2) {
                tempGameObject = gameObjects[i];
                gameObjects[i] = gameObjects[i + 1];
                gameObjects[i + 1] = tempGameObject;

                int j = i;
                while (j > 0 && gameObjects[j].x - gameObjects[j].boundingDimension / 2 
                        < gameObjects[j - 1].x - gameObjects[j - 1].boundingDimension / 2) {
                ...
                j = i + 1;
                while (j < size - 1 && gameObjects[j].x - gameObjects[j].boundingDimension / 2 
                        > gameObjects[j + 1].x - gameObjects[j + 1].boundingDimension / 2) {
                    tempGameObject = gameObjects[j];
                    gameObjects[j] = gameObjects[j + 1];
                    gameObjects[j + 1] = tempGameObject;
    ...
    private void incrementalSortGameObjectsOnYPosition() {
        ...
        GameObject tempGameObject;
        for (int i = 0; i < size - 1; i++) {
            if (gameObjects[i].y - gameObjects[i].boundingDimension / 2 
                    > gameObjects[i + 1].y - gameObjects[i + 1].boundingDimension / 2) {
                tempGameObject = gameObjects[i];
                gameObjects[i] = gameObjects[i + 1];
                gameObjects[i + 1] = tempGameObject;

                int j = i;
                while (j > 0 && gameObjects[j].y - gameObjects[j].boundingDimension / 2 
                        < gameObjects[j - 1].y - gameObjects[j - 1].boundingDimension / 2) {
                    tempGameObject = gameObjects[j];
                    gameObjects[j] = gameObjects[j - 1];
                    gameObjects[j - 1] = tempGameObject;
                    j--;
                ...
                j = i + 1;
                while (j < size - 1 && gameObjects[j].y - gameObjects[j].boundingDimension / 2 
                        > gameObjects[j + 1].y - gameObjects[j + 1].boundingDimension / 2) {
                    tempGameObject = gameObjects[j];
                    gameObjects[j] = gameObjects[j + 1];
                    gameObjects[j + 1] = tempGameObject;
    ...
    /*
     * The following code has not been tested - it is intended to return the 
     * leftmost index that corresponds to a specific location (i.e. x location)
     * and is intended to enable forward searching from this position. It 
     * assumes that the game object collection has been correctly sorted
     * prior to call
    public int getSortedLeftmostIdxForLocation(double location) {
        int bottomIdx = -1;
        int middleIdx = -1;
        int topIdx = size;
        ...
        if (sortOrder == SortOrder.X) {
            while (topIdx - bottomIdx > 1) {
                ...
                if (gameObjects[middleIdx].x - gameObjects[middleIdx].width / 2.0 < location) {
                    bottomIdx = middleIdx;
                } else {
                    topIdx = middleIdx;
                }
            ...
            while (middleIdx > 0 && (gameObjects[middleIdx - 1].x - gameObjects[middleIdx - 1].width / 2.0) >= location) {
                middleIdx--;
            }
        } else {
            while (topIdx - bottomIdx > 1) {
                middleIdx = (topIdx + bottomIdx) / 2;
                if (gameObjects[middleIdx].y - gameObjects[middleIdx].height / 2.0 < location) {
                    bottomIdx = middleIdx;
                } else {
                    topIdx = middleIdx;
                }
            ...
            while (middleIdx > 0 && (gameObjects[middleIdx - 1].y - gameObjects[middleIdx - 1].height / 2.0) >= location) {
                middleIdx--;
            }
        }
