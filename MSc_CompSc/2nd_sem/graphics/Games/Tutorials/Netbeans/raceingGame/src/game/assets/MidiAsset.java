...
public class MidiAsset extends Asset {
    ...
    public MidiAsset(String assetName, URL midiURL, boolean continuallyPlay) {
        ...
        if (midiURL == null) {
            throw new NullPointerException("MidiAsset.constructor: " +
                    "NULL midi URL specified.");
        }
    ...
    private Sequence loadMidiSequence(URL sequenceName) {
        ...
        try {
            ...
            throw new IllegalArgumentException("MidiAsset.loadMidiSequence: " + 
    ...
    private void initialiseSequencer() {
        try {
            ...
            if (sequencerPos != -1) {
                sequencer = (Sequencer) MidiSystem.getMidiDevice( 
                        midiDeviceInfo[sequencerPos] );
            }
            ...
            // Now that a sequencer has been found, open it
            sequencer.open();

            // Attempt to locate and open a suitable synthesizer for the sequencer
    ...
    public Asset deepClone() {
        if (midiURL == null) {
            throw new IllegalStateException("MidiAsset.deepClone: " +
                    "Object constructed without midi URL specified. " +
