...
public class AssetLoader {
     ...
     */
    public final BufferedImage[] loadImageSegments(
        ...
        for (int iIdx = 0; iIdx < imageSegments.length; iIdx++)
            imageSegments[iIdx] = extractImageSegment( baseImage, 
                    imageSegmentRectangles[iIdx].x, 
                    imageSegmentRectangles[iIdx].y, 
                    imageSegmentRectangles[iIdx].width, 
                    imageSegmentRectangles[iIdx].height);
    ...
    public final BufferedImage extractImageSegment(
        ...
        if (sourceImage == null)
            throw new NullPointerException(
                    "AssetLoader.extractImageSegement: NULL source image specified.");

        // Test to ensure the image segment is valid
        if (x < 0 || y < 0 || width <= 0 || height <= 0)
            throw new IllegalArgumentException(
                    "AssetLoader.extractImageSegment: Invalid image segment " +
                    "x =" + x + " y =" + y + " width = " + width + " height = " + height);

        // Test to ensure the image segment can be extracted from the source image
        if (x + width > sourceImage.getWidth() || y + height > sourceImage.getHeight())
            throw new IllegalArgumentException(
                    "AssetLoader.extractImageSegment: " + 
                    "Segment cannot be extracted from source image" + "Segment x =" + 
                    x + " y =" + y + " width = " + width + " height = " + height + 
                    ": Image width = " + sourceImage.getWidth() + " height = " +
                    sourceImage.getHeight());

        // Create a compatible image to store the extracted image segment
        extractedImage = graphicsConfiguration.createCompatibleImage(
                width, height, sourceImage.getColorModel().getTransparency());

        // Extract and store the image segment within the comptiable image
        Graphics2D graphics2D = extractedImage.createGraphics();
        graphics2D.drawImage(
                sourceImage, 0, 0, width, height, x, y, x + width, y + height, null);
        graphics2D.dispose();
    ...
    public final Sequence loadMidiSequence(URL sequenceName) {
        ...
        try {
            // Attempt to load the specified MIDI sequence
            sequence = MidiSystem.getSequence(sequenceName);
        } catch (InvalidMidiDataException exception) {
            throw new IllegalArgumentException("AssetLoader.loadMidiSequence: " + 
                    "Unreadable or unsupported Midi file format " + sequenceName);
        } catch (IOException exception) {
            throw new IllegalArgumentException("AssetLoader.loadMidiSequence: " + 
                    "IO Exception for " + sequenceName);
        }
