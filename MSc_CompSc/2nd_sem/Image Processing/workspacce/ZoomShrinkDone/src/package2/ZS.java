...
public class ZS extends JPanel
{
    ...
    BufferedImage imageNew;
   	...
   	public static double scale_x = 1; // zoom factor for x-cordinate
    public static double scale_y=1;   //zoom factor for y-cordinate
    
   	public static double shrink_x = 1;  //shrink_factor for x-cordinate
	public static double shrink_y = 1;  //shrink_factor for y-cordinate
	...
	public void LoadBuffer()
	{
		...
		if(flag == 1)
        {	
        	this.imageNew = new BufferedImage(imageOld.getWidth()*(int)scale_x,imageOld.getHeight()*(int)scale_y,BufferedImage.TYPE_INT_RGB);
        }
        if (flag == 2)
        {	
        	this.imageNew = new BufferedImage(imageOld.getWidth()/(int)shrink_x,imageOld.getHeight()/(int)shrink_y,BufferedImage.TYPE_INT_RGB);
        }
        ...
        for(int i=0;i<x;i++)
         {
          for(int j=0;j<y;j++)
            {
             val = imageOld.getRGB(i/(int)scale_x, j/(int)scale_x);//returns the rgb color of pixel at position(x,y)
         ...
         for(int i=0;i<x;i++)
           {
            for(int j=0;j<y;j++)
            	{
            		val = imageOld.getRGB(i*(int)shrink_x, j*(int)shrink_y);//returns the rgb color of pixel at position(x,y)
