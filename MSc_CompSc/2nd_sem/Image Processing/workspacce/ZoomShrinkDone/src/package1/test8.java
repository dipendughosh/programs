...
public class test8 extends JPanel implements KeyListener
{
	...
	public static double shrinkx = 1;
	//Shrink factor in the y direction
	public static double shrinky = 1;
    ...
    public void LoadBuffer()
	{
    	...
    	if(flagz == 1)
        {
    		try
        	{
    			zx=(int)(imageOld.getWidth()*zoomx);
    			zy=(int)(imageOld.getHeight()*zoomy);
    			this.imageNew = new BufferedImage(zx,zy,BufferedImage.TYPE_INT_RGB);
        	}
    		catch(Exception e)
    		{
    		}
        ...
        if(flags == 1)
        {
        	try
        	{
        		sx=(int)(imageOld.getWidth()/shrinkx);
        		sy=(int)(imageOld.getHeight()/shrinky);
        		this.imageNew = new BufferedImage(sx,sy,BufferedImage.TYPE_INT_RGB);
        	}
        	catch(Exception e)
        	{
        	}
	...
	{
		int i=0,j=0,x=0,y=0,valu=0;
		
		for(i=1;i<imageNew.getHeight();i++)
		{
			for(j=1;j<imageNew.getWidth();j++)
			{
				valu=0;
				for(x=i-1;x<=i+1;x++)
					for(y=j-1;y<=j+1;y++)
						valu=valu+imageNew.getRGB(x,y);
				valu=valu/9;
