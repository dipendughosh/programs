/*Reads a '.ppm' file in the RAW format and converts to ASCII '.pgm. format*/
...
//Class to convert PPM RAW to PGM ASCII
...
void ConvertFromPPMBtoPPMA::openFiles()
{
	...
	fin=fopen("C:\\images\\image2.ppm","rb");
	//Opening the ASCII file in normal WRITE mode for writing
	fout=fopen("C:\\images\\image2_ascii.pgm","w");
}
...
void ConvertFromPPMBtoPPMA::readwrite()
{
	...
	for(i=0;i<4*ht;i++) 
	{
		...
		{	
			...
			//Writing a new line after every 24th ASCII charachter
			if(charCntr > 24)
			{
				fprintf(fout,"%s","\n");
				charCntr=0;
