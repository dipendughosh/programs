...
<div class="wpmd">
<div align=center><font color="#000080" face="Algerian" class="ws24"><B><U>BOOKING RECEIPT</U></B></font></div>
</div></div>

<div id="html1" style="position:absolute; overflow:hidden; left:270px; top:320px; width:550px; height:550px; z-index:13">
<pre>
<% if(request.getAttribute("reserved")=="true") { %>
TOUR TO : <%=request.getAttribute("tourName") %>
ON DATE : <%=request.getAttribute("onDate") %>
ADULTS  : <%=request.getAttribute("adults") %>
CHILDREN: <%=request.getAttribute("children") %>

TOTAL AMOUNT : <%=request.getAttribute("amount") %>    : Discount Rs.2000/- per child
REPORTING TIME : 2 Hours Before The Time Of Departure

<%}
 else %>
 SORRY THE REQUIRED SEATS ARE NOT AVAILABLE : CHECK AVAILABILITY PLEASE
</pre>
