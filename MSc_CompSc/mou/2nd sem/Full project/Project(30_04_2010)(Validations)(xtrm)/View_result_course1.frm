...
Dim i As Integer
    ...
    Set objfile = objFSO.CreateTextFile("c:\result_course.txt")
    objfile.Close
    Set objTextFile = objFSO.OpenTextFile("c:\result_course.txt", ForAppending, True)
...
Private Sub Form_Load()
    ...
    Set rs = con.OpenResultset("SELECT R_C_ID,COUNT(*) AS C,MAX(R_MARKS) AS M FROM RESULT GROUP BY R_C_ID ORDER BY R_C_ID;", rdOpenStatic)
    Set rs1 = con.OpenResultset("SELECT COUNT(*) AS A FROM RESULT WHERE R_MARKS > 40 GROUP BY R_C_ID ORDER BY R_C_ID;", rdOpenStatic)
    While rs.EOF = False
